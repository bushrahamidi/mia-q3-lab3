...
app.put('/api/tasks/:id', (req, res) => {
  try {
    ...
    stmt.run(title, description || '', due_date || null, priority || existing.priority, req.params.id);
...
app.patch('/api/tasks/:id', (req, res) => {
  try {
    ...
    if (completed === undefined && priority === undefined) {
      return res.status(400).json({ error: 'Completed or priority must be provided' });
    }
